...
You can run the program, and the agent alongside it, and get the agent to tell you what things were reflected on, serialized, proxied, etc., by running the app on the JVM with `-Dagent=true` when running the app with maven (try `spring-boot:run). This will dump out config files in the `target/native/agent-output` directory. You can inspect those and then use those to tell you what could you'll need to contribute hints for. The Java agent dumps out _everything_, including some things that Spring's AOT engine will do automatically for you and thus you don't need to reproduce. I'd start with all the mentions of types across the various configuration files that are in the same packages as code in your application.
